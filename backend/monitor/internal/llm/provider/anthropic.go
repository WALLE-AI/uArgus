...
import (
	...
	"github.com/WALL-AI/uArgus/backend/monitor/internal/llm/prompt"
)
...
func (p *AnthropicProvider) CallModel(ctx context.Context, params CallModelParams) (<-chan llm.StreamEvent, error) {
...
func (p *AnthropicProvider) streamResponse(ctx context.Context, params anthropic.MessageNewParams, callParams CallModelParams, ch chan<- llm.StreamEvent) {
	const maxRetries = 2
	var lastErr error
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		errorMsg := llm.Message{
			...
			Content: []llm.ContentBlock{
				{Type: llm.ContentBlockText, Text: errorContent},
			},
			Timestamp: time.Now(),
		}
		ch <- llm.StreamEvent{
...
func (p *AnthropicProvider) doStream(ctx context.Context, params anthropic.MessageNewParams, callParams CallModelParams, ch chan<- llm.StreamEvent) error {
...
func (p *AnthropicProvider) processStreamEvent(
	...
	case "content_block_stop":
		if len(*contentBlocks) > 0 {
			lastBlock := &(*contentBlocks)[len(*contentBlocks)-1]
			if lastBlock.Type == llm.ContentBlockToolUse {
				ch <- llm.StreamEvent{
					Type: llm.EventToolUse,
					ToolUse: &llm.ToolUseEvent{
						ID:    lastBlock.ID,
						Name:  lastBlock.Name,
...
func convertMessageToAnthropic(msg llm.Message) (anthropic.MessageParam, error) {
	switch msg.Type {
	...
	case llm.MessageTypeUser:
		...
		for _, b := range msg.Content {
			...
			case llm.ContentBlockText:
				blocks = append(blocks, anthropic.NewTextBlock(b.Text))
			case llm.ContentBlockToolResult:
